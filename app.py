...
def panel_registros_en_formulario(prefix):
    tipo = "estudiante" if "estudiante" in str(prefix).lower() else "hogar"

    with st.expander("📥 Revisar / cargar entrevista existente al formulario", expanded=False):
        panel_cargar_registro_al_formulario(prefix=f"{prefix}_cargar", tipo=tipo)

    with st.expander("🗑️ Borrar entrevistas seleccionadas", expanded=False):
        st.caption("Sistema de borrado seguro activo: crea respaldo antes de eliminar entrevistas.")
        selector_ver_y_borrar_registros(prefix)




def obtener_valor_fila_registro(fila, posibles_columnas, defecto=""):
    """
    Obtiene un valor desde una fila de registro usando nombres alternativos de columnas.
    """
    for col in posibles_columnas:
        if col in fila and str(fila.get(col, "")).strip():
            return str(fila.get(col, "")).strip()
    return defecto


def cargar_registro_en_session_state(fila, tipo="hogar"):
    """
    Carga una entrevista guardada al formulario actual.
    No guarda, no borra, solo prellena campos para revisar/corregir/completar.
    """
    if tipo == "hogar":
        pref = "hogar"
        st.session_state["antecedentes_hogar"] = obtener_valor_fila_registro(
            fila, ["Antecedentes", "Antecedentes_relevantes", "Motivo_detallado", "Relato", "Descripcion"]
        )
        st.session_state["antecedentes_ordenados_hogar"] = st.session_state["antecedentes_hogar"]

        st.session_state["acuerdos_hogar"] = obtener_valor_fila_registro(
            fila, ["Acuerdos", "Acuerdos_y_conclusiones", "Conclusiones"]
        )
        st.session_state["acuerdos_hogar_mejorado"] = st.session_state["acuerdos_hogar"]

        st.session_state["compromisos_hogar"] = obtener_valor_fila_registro(
            fila, ["Compromisos", "Compromisos_familia", "Compromisos_apoderado"]
        )

        st.session_state["resumen_hogar"] = obtener_valor_fila_registro(
            fila, ["Resumen_libro", "Resumen", "Registro_libro"]
        )

        # Campos generales si existen en el formulario
        for key, cols in {
            "hogar_fecha": ["Fecha_entrevista", "Fecha"],
            "hogar_hora": ["Hora", "Hora_entrevista"],
            "hogar_motivo_manual": ["Motivo"],
            "hogar_apod_manual": ["Apoderado", "Nombre_apoderado"],
            "hogar_est_manual": ["Estudiante", "Estudiante(s)", "Nombre_estudiante"],
            "hogar_curso_manual": ["Curso"],
        }.items():
            valor = obtener_valor_fila_registro(fila, cols)
            if valor:
                st.session_state[key] = valor

    else:
        pref = "estudiante"
        st.session_state["antecedentes_estudiante"] = obtener_valor_fila_registro(
            fila, ["Antecedentes", "Antecedentes_relevantes", "Motivo_detallado", "Relato", "Descripcion"]
        )
        st.session_state["antecedentes_ordenados_estudiante"] = st.session_state["antecedentes_estudiante"]

        st.session_state["acuerdos_estudiante"] = obtener_valor_fila_registro(
            fila, ["Acuerdos", "Acuerdos_y_conclusiones", "Conclusiones"]
        )
        st.session_state["acuerdos_estudiante_mejorado"] = st.session_state["acuerdos_estudiante"]

        st.session_state["compromisos_estudiante"] = obtener_valor_fila_registro(
            fila, ["Compromisos", "Compromisos_estudiante"]
        )

        st.session_state["resumen_estudiante"] = obtener_valor_fila_registro(
            fila, ["Resumen_libro", "Resumen", "Registro_libro"]
        )

        for key, cols in {
            "estudiante_fecha": ["Fecha_entrevista", "Fecha"],
            "estudiante_hora": ["Hora", "Hora_entrevista"],
            "estudiante_motivo_manual": ["Motivo"],
            "estudiante_est_manual": ["Estudiante", "Estudiante(s)", "Nombre_estudiante"],
            "estudiante_curso_manual": ["Curso"],
        }.items():
            valor = obtener_valor_fila_registro(fila, cols)
            if valor:
                st.session_state[key] = valor

    # Datos compartidos / trazabilidad
    for key, cols in {
        "registro_cargado_cc": ["CC"],
        "registro_cargado_numero_entrevista": ["Numero_entrevista", "N_entrevista"],
        "registro_cargado_estado": ["Estado_institucional", "Estado_caso", "Estado"],
        "registro_cargado_motivo": ["Motivo"],
        "registro_cargado_protocolo": ["Protocolo", "Protocolos"],
        "registro_cargado_falta_rice": ["Falta_RICE", "Tipo_registro", "Tipo_falta", "Falta"],
    }.items():
        valor = obtener_valor_fila_registro(fila, cols)
        if valor:
            st.session_state[key] = valor

    st.session_state["registro_cargado_desde_excel"] = True
    st.session_state["registro_cargado_tipo"] = tipo


def panel_cargar_registro_al_formulario(prefix="cargar", tipo="hogar"):
    """
    Permite seleccionar UN registro guardado y cargar todos sus detalles al formulario.
    No elimina datos.
    """
    st.markdown("### Cargar entrevista existente al formulario")
    df = leer_registro_entrevistas()

    if df.empty:
        st.info("No hay registros guardados para cargar.")
        return

    df_vista = df.copy().reset_index(drop=False)
    df_vista = df_vista.rename(columns={"index": "_indice_original"})
    df_vista.insert(0, "Cargar", False)
    ...
    st.info("Marque solo UNA entrevista en la columna Cargar. Luego presione el botón para completar el formulario.")

    editado = st.data_editor(
        df_vista,
        use_container_width=True,
        hide_index=True,
        key=f"{prefix}_{tipo}_editor_cargar_un_registro",
        column_config={
            "Cargar": st.column_config.CheckboxColumn(
                "Cargar",
                help="Seleccione solo una entrevista para cargarla al formulario.",
                default=False,
            )
        },
        disabled=[c for c in df_vista.columns if c != "Cargar"],
    )

    seleccionados = editado[editado["Cargar"] == True].copy()

    if seleccionados.empty:
        st.caption("No hay entrevista seleccionada.")
        return

    if len(seleccionados) > 1:
        st.error("Seleccione solo UNA entrevista para cargar al formulario.")
        return

    idx = int(seleccionados.iloc[0]["_indice_original"])
    fila = df.loc[idx].to_dict()

    with st.expander("Vista completa de la entrevista seleccionada", expanded=True):
        for k, v in fila.items():
            st.write(f"**{k}:** {v}")

    if st.button("📥 Cargar registro seleccionado al formulario", key=f"{prefix}_{tipo}_btn_cargar_registro"):
        cargar_registro_en_session_state(fila, tipo=tipo)
        st.success("Registro cargado al formulario. Revise, corrija o complete antes de volver a guardar.")
        st.rerun()
...
def aviso_registro_cargado():
    if st.session_state.get("registro_cargado_desde_excel"):
        st.info(
            "Hay una entrevista existente cargada desde registros. Revise/corrija la información antes de guardar nuevamente. "
            f"CC: {st.session_state.get('registro_cargado_cc', '')} | "
            f"N° entrevista: {st.session_state.get('registro_cargado_numero_entrevista', '')}"
        )
