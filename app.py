...
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw4bmWX1E1ArZNSVdsngYV0GnsXgFBJ2d6wf74S9_GtUkNqqQlUujXQ28kFBcCf4UkW/exec"
...
if menu == "Guardado Manual Google Sheets":
    pagina_guardado_manual_google_sheets()
...
if menu == "Inicio":
    ...
    datos = {
        **datos_previos,
        ...
        "antecedentes": locals().get("antecedentes_para_word", locals().get("antecedentes", "")),
        "acuerdos": locals().get("acuerdos_word_final", locals().get("acuerdos", "")),
        "compromisos": locals().get("compromisos", ""),
        "resumen_libro": locals().get("resumen_libro", ""),
        "observaciones": locals().get("observaciones", ""),
    }

    st.divider()
    st.info("Para registrar oficialmente la entrevista, presione el botón verde: 💾 GUARDAR ENTREVISTA EN GOOGLE SHEETS.")


    st.markdown("---")
    st.error("GUARDADO OFICIAL: antes de salir, presione este botón para registrar en Google Sheets.")
    if st.button("💾 GUARDAR ENTREVISTA AHORA EN GOOGLE SHEETS", key="guardar_google_sheets_visible_unico", type="primary", use_container_width=True):
        try:
            guardar_en_google_sheets_directo(datos)
        except NameError:
            st.error("Este botón no pudo leer los datos del formulario en esta sección. Use el menú lateral: Guardado Manual Google Sheets.")
        except Exception as e:
            st.error(f"No fue posible guardar: {e}")
    st.markdown("---")

    col_guardar, col_word = st.columns(2)

    with col_guardar:
        if st.button("💾 GUARDAR ENTREVISTA EN GOOGLE SHEETS", key="guardar_hogar", type="primary"):
            if not estudiante:
    ...
    with col_guardar:
        if st.button("💾 GUARDAR ENTREVISTA EN GOOGLE SHEETS", key="guardar_estudiante", type="primary"):
            if not estudiante:
                st.error("Debes seleccionar estudiante.")
            elif datos.get("estado_caso") == "Cerrado" and datos.get("pendientes_checklist"):
                st.error("No se puede registrar como Cerrado porque existen pasos pendientes en el checklist.")
