...
if menu == "Inicio":
    ...
    st.divider()
    st.markdown("### 💾 Guardado oficial de la entrevista")
    st.warning("Presione este botón para guardar la entrevista en Google Sheets. El botón de Word solo descarga el documento, NO registra la entrevista.")

    if st.button("💾 GUARDAR ENTREVISTA AHORA EN GOOGLE SHEETS", key="guardar_hogar_principal", use_container_width=True):
        if not estudiante:
    ...
    st.caption("Después de guardar, revise la fila nueva en Google Sheets.")

    col_guardar, col_word = st.columns(2)

    with col_guardar:
        st.info("Guardado principal disponible arriba.")
    ...
    if st.button("💾 GUARDAR ENTREVISTA AHORA EN GOOGLE SHEETS", key="guardar_estudiante_principal", use_container_width=True):
        if not estudiante:
            st.error("Debes seleccionar estudiante.")
        elif datos.get("estado_caso") == "Cerrado" and datos.get("pendientes_checklist"):
            st.error("No se puede registrar como Cerrado porque existen pasos pendientes en el checklist.")
