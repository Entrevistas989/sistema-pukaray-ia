...
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxXo082qcDnryZ9xvcG-bizmTjajEPFJYuV8W1p_f8rbkTG--rea4wB8MpernGjFkDT/exec"
...
def crear_respaldo_registro_entrevistas(motivo="respaldo"):
    """
    Crea respaldo fisico del Excel de registros antes de operaciones criticas.
    """
    ...
    try:
        ...
        carpeta_respaldos = DATOS_DIR / "respaldos_registro_entrevistas"
        carpeta_respaldos.mkdir(parents=True, exist_ok=True)

        timestamp = _dt.now().strftime("%Y%m%d_%H%M%S")
        destino = carpeta_respaldos / f"registro_entrevistas_{motivo}_{timestamp}.xlsx"

        _shutil.copy2(REGISTRO_EXCEL, destino)
        return str(destino)
    except Exception as e:
        st.error(f"No fue posible crear respaldo previo: {e}")
        return ""


def guardar_registro_seguro(df_nuevo, motivo="actualizacion"):
    """
    Guarda registro_entrevistas.xlsx con respaldo previo obligatorio.
    """
    respaldo = crear_respaldo_registro_entrevistas(motivo=motivo)

    if REGISTRO_EXCEL.exists() and not respaldo:
        st.error("Operacion cancelada: no se pudo crear respaldo previo del registro.")
        return False

    try:
        REGISTRO_EXCEL.parent.mkdir(parents=True, exist_ok=True)
        ...
        if respaldo:
            st.success(f"Respaldo creado antes de guardar: {respaldo}")
        return True
    except Exception as e:
        st.error(f"No fue posible guardar el registro actualizado: {e}")
        return False
...
def selector_ver_y_borrar_registros(prefix="global"):
    ...
    - Seleccionar una o varias entrevistas por casilla.
    - Borrar SOLO entrevistas seleccionadas.
    - Crea respaldo automatico antes de borrar.
    ...
    if df.empty:
        st.info("Aun no existen registros guardados.")
        return

    st.caption("Este panel permite revisar entrevistas y borrar solo registros seleccionados. No borra casos completos.")

    df_vista = df.copy().reset_index(drop=False)
    df_vista = df_vista.rename(columns={"index": "_indice_original"})
    df_vista.insert(0, "Seleccionar", False)
    ...
    columnas_existentes = [c for c in columnas_prioritarias if c in df_vista.columns]
    otras_columnas = [c for c in df_vista.columns if c not in columnas_existentes]
    df_vista = df_vista[columnas_existentes + otras_columnas]

    st.markdown("#### Seleccionar entrevistas")
    st.info("Marque una o varias filas en la columna Seleccionar. Luego revise el detalle antes de borrar.")

    editado = st.data_editor(
        df_vista,
        use_container_width=True,
        hide_index=True,
        key=f"{prefix}_editor_borrado_seguro",
        column_config={
            "Seleccionar": st.column_config.CheckboxColumn(
                "Seleccionar",
                help="Marque solo las entrevistas que desea eliminar.",
                default=False,
            )
        },
    ...
    with st.expander("Ver tabla completa sin edicion", expanded=False):
        st.dataframe(df, use_container_width=True)

    if seleccionados.empty:
        st.caption("No hay entrevistas seleccionadas para borrar.")
        return

    st.warning(f"Entrevistas seleccionadas para posible eliminacion: {len(seleccionados)}")

    with st.expander("Revisar detalle de entrevistas seleccionadas", expanded=True):
        st.dataframe(seleccionados.drop(columns=["Seleccionar"], errors="ignore"), use_container_width=True)

    st.markdown("#### Confirmacion obligatoria")
    st.error("Antes de borrar se creara un respaldo automatico del Excel actual. Esta accion eliminara solo las entrevistas seleccionadas.")

    confirmacion = st.text_input(
        "Para confirmar escriba exactamente: BORRAR",
        key=f"{prefix}_texto_confirmacion_borrar"
    )

    if st.button("Borrar entrevistas seleccionadas", key=f"{prefix}_btn_borrar_varios_seguro"):
        if confirmacion.strip().upper() != "BORRAR":
            st.error("Operacion cancelada. Debe escribir BORRAR para confirmar.")
            return

        try:
            indices_a_borrar = seleccionados["_indice_original"].astype(int).tolist()
            df_nuevo = df.drop(index=indices_a_borrar).reset_index(drop=True)

            ok = guardar_registro_seguro(df_nuevo, motivo="antes_de_borrar")

            if ok:
                st.success(f"Se eliminaron {len(indices_a_borrar)} entrevista(s) seleccionada(s).")
                st.cache_data.clear()
                st.rerun()
        except Exception as e:
            st.error(f"No fue posible borrar las entrevistas seleccionadas: {e}")
...
def panel_revision_individual_entrevistas(prefix="revision"):
    st.markdown("### 📂 Revisar entrevistas guardadas")
    df = leer_registro_entrevistas()

    if df.empty:
        st.info("No existen entrevistas registradas.")
        return

    df_vista = df.copy().reset_index(drop=False)
    df_vista = df_vista.rename(columns={"index":"_idx"})
    ...
    editado = st.data_editor(
        ...
        use_container_width=True,
        hide_index=True,
        key=f"{prefix}_revision_editor",
        disabled=[c for c in df_vista.columns if c != "Ver"]
    )

    seleccion = editado[editado["Ver"] == True]

    if len(seleccion) == 0:
        return

    if len(seleccion) > 1:
        st.warning("Seleccione solo una entrevista.")
        return

    idx = int(seleccion.iloc[0]["_idx"])
    fila = df.iloc[idx].to_dict()

    st.markdown("## Ficha completa de entrevista")
...
def panel_registros_en_formulario(prefix):
    with st.expander("📂 Revisar entrevistas guardadas", expanded=False):
        panel_revision_individual_entrevistas(prefix)

    with st.expander("🗑️ Borrar entrevistas seleccionadas", expanded=False):
        selector_ver_y_borrar_registros(prefix)




def normalizar_registro_para_google_sheets(registro):
    """Convierte el registro interno al formato esperado por Apps Script / Google Sheets."""
    def val(*claves):
        # 1. Buscar primero en el registro construido por el formulario
        for clave in claves:
            if clave in registro and str(registro.get(clave, "")).strip():
                try:
                    return limpiar_html_editor(registro.get(clave, ""))
                except Exception:
                    return str(registro.get(clave, "")).strip()

        # 2. Respaldo: buscar directamente en session_state de los widgets
        # Esto evita que antecedentes/acuerdos/compromisos queden en blanco
        # si el formulario no los pasó correctamente al diccionario datos.
        for clave in claves:
            posibles_keys = [
                clave,
                clave.lower(),
                f"hp_{clave.lower()}_editor",
                f"e_{clave.lower()}_editor",
                f"{clave.lower()}_hogar",
                f"{clave.lower()}_estudiante",
            ]
            for k in posibles_keys:
                if k in st.session_state and str(st.session_state.get(k, "")).strip():
                    try:
                        return limpiar_html_editor(st.session_state.get(k, ""))
                    except Exception:
                        return str(st.session_state.get(k, "")).strip()

        return ""

    return {
        "CC": val("CC", "cc"),
        "Folio": val("Folio", "folio"),
        "Numero_entrevista": val("Numero_entrevista", "numero_entrevista", "N_entrevista", "n_entrevista"),
        "Fecha_registro_sistema": val("Fecha_registro_sistema") or datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
        "Fecha_entrevista": val("Fecha_entrevista", "fecha"),
        "Hora": val("Hora", "hora"),
        "Tipo_registro": val("Tipo_registro", "tipo_registro") or "Entrevista",
        "Departamento_que_cita": val("Departamento_que_cita", "departamento", "depto_cita"),
        "Curso": val("Curso", "curso"),
        "Estudiante": val("Estudiante", "estudiante", "Estudiante(s)"),
        "RUN": val("RUN", "run"),
        "Apoderado": val("Apoderado", "apoderado"),
        "Vinculo": val("Vinculo", "Vínculo", "vinculo"),
        "Motivo": val("Motivo", "motivo"),
        "Tipo_registro_RICE": val("Tipo_registro_RICE", "tipo_registro_rice"),
        "Tipo_falta_categoria_entorno_tipo": val("Tipo_falta_categoria_entorno_tipo", "tipo_falta"),
        "Estado_caso": val("Estado_caso", "estado_caso"),
        "Condicion_caso": val("Condicion_caso", "condicion_caso"),
        "Estado_institucional": val("Estado_institucional", "estado_institucional"),
        "Detalle_estado": val("Detalle_estado", "detalle_estado"),
        "Protocolos_aplicados": val("Protocolos_aplicados", "protocolos_aplicados"),
        "Antecedentes": val("Antecedentes", "antecedentes", "antecedentes_para_word", "motivo_word"),
        "Acuerdos": val("Acuerdos", "acuerdos", "acuerdos_word"),
        "Compromisos": val("Compromisos", "compromisos"),
        "Resumen_libro": val("Resumen_libro", "resumen_libro", "Resumen", "resumen"),
        "Observaciones": val("Observaciones", "observaciones"),
        "Checklist_cierre": val("Checklist_cierre", "checklist_cierre"),
        "Pendientes_checklist": val("Pendientes_checklist", "pendientes_checklist"),
        "Usuario_registro": val("Usuario_registro") or st.session_state.get("usuario", "")
    }


def enviar_registro_google_sheets(registro):
    """Envía una entrevista a Google Sheets mediante Apps Script."""
    if not APPS_SCRIPT_URL:
        st.warning("Google Sheets no está configurado: falta APPS_SCRIPT_URL.")
        return False

    payload = normalizar_registro_para_google_sheets(registro)
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as response:
            respuesta = response.read().decode("utf-8", errors="ignore").strip()

        if "OK" in respuesta.upper():
            st.success("Registro guardado correctamente en Google Sheets.")
            return True

        st.warning(f"Google Sheets respondió: {respuesta}")
        return False
...
def valor_google_seguro(datos, *claves):
    """
    Obtiene valores desde datos o session_state.
    Evita pérdida de antecedentes/acuerdos/compromisos por diferencias de keys.
    """
    for clave in claves:
        if isinstance(datos, dict) and clave in datos and str(datos.get(clave, "")).strip():
            return limpiar_html_editor(datos.get(clave, ""))
    ...
    for clave in claves:
        ...
        keys_session.extend([
            base,
            base.lower(),
            f"hp_{base.lower()}_editor",
            f"e_{base.lower()}_editor",
            f"{base.lower()}_hogar",
            f"{base.lower()}_estudiante",
            f"{base.lower()}_ordenados_hogar",
            f"{base.lower()}_ordenados_estudiante",
            f"{base.lower()}_hogar_mejorado",
            f"{base.lower()}_estudiante_mejorado",
        ])

    # Keys reales usadas por la app
    keys_session.extend([
        "hp_antecedentes_ordenados_editor",
        "hp_acuerdos_editor",
        "hp_compromisos_editor",
        "e_antecedentes_ordenados_editor",
        "e_acuerdos_editor",
        "e_compromisos_editor",
        "antecedentes_ordenados_hogar",
        "acuerdos_hogar_mejorado",
        "compromisos_hogar_mejorado",
        "antecedentes_ordenados_estudiante",
        "acuerdos_estudiante_mejorado",
        "compromisos_estudiante_mejorado",
        "resumen_hogar",
        "resumen_estudiante",
        "observaciones_hogar",
    ...
    for key in keys_session:
        if key in st.session_state and str(st.session_state.get(key, "")).strip():
            return limpiar_html_editor(st.session_state.get(key, ""))

    return ""


def construir_payload_google_sheets(datos):
    """
    Construye la fila para Google Sheets con los nombres exactos esperados por Apps Script.
    """
    return {
        "CC": valor_google_seguro(datos, "CC", "cc"),
        "Folio": valor_google_seguro(datos, "Folio", "folio"),
        "Numero_entrevista": valor_google_seguro(datos, "Numero_entrevista", "numero_entrevista", "N_entrevista"),
        "Fecha_registro_sistema": datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
        "Fecha_entrevista": valor_google_seguro(datos, "Fecha_entrevista", "fecha"),
        "Hora": valor_google_seguro(datos, "Hora", "hora"),
        "Tipo_registro": valor_google_seguro(datos, "Tipo_registro", "tipo_registro") or "Entrevista",
        "Departamento_que_cita": valor_google_seguro(datos, "Departamento_que_cita", "departamento"),
        "Curso": valor_google_seguro(datos, "Curso", "curso"),
        "Estudiante": valor_google_seguro(datos, "Estudiante", "estudiante"),
        "RUN": valor_google_seguro(datos, "RUN", "run"),
        "Apoderado": valor_google_seguro(datos, "Apoderado", "apoderado"),
        "Vinculo": valor_google_seguro(datos, "Vinculo", "Vínculo", "vinculo"),
        "Motivo": valor_google_seguro(datos, "Motivo", "motivo"),
        "Tipo_registro_RICE": valor_google_seguro(datos, "Tipo_registro_RICE", "tipo_registro_rice"),
        "Tipo_falta_categoria_entorno_tipo": valor_google_seguro(datos, "Tipo_falta_categoria_entorno_tipo", "tipo_falta"),
        "Estado_caso": valor_google_seguro(datos, "Estado_caso", "estado_caso"),
        "Condicion_caso": valor_google_seguro(datos, "Condicion_caso", "condicion_caso"),
        "Estado_institucional": valor_google_seguro(datos, "Estado_institucional", "estado_institucional"),
        "Detalle_estado": valor_google_seguro(datos, "Detalle_estado", "detalle_estado"),
        "Protocolos_aplicados": valor_google_seguro(datos, "Protocolos_aplicados", "protocolos_aplicados"),
        "Antecedentes": valor_google_seguro(datos, "Antecedentes", "antecedentes", "antecedentes_para_word", "motivo_word"),
        "Acuerdos": valor_google_seguro(datos, "Acuerdos", "acuerdos", "acuerdos_word"),
        "Compromisos": valor_google_seguro(datos, "Compromisos", "compromisos"),
        "Resumen_libro": valor_google_seguro(datos, "Resumen_libro", "resumen_libro", "resumen"),
        "Observaciones": valor_google_seguro(datos, "Observaciones", "observaciones"),
        "Checklist_cierre": valor_google_seguro(datos, "Checklist_cierre", "checklist_cierre"),
        "Pendientes_checklist": valor_google_seguro(datos, "Pendientes_checklist", "pendientes_checklist"),
        "Usuario_registro": st.session_state.get("usuario", "")
    }


def guardar_en_google_sheets_directo(datos):
    """
    Envía directamente la entrevista a Google Sheets.
    """
    try:
        payload = construir_payload_google_sheets(datos)
        data = json.dumps(payload, ensure_ascii=False).encode("utf-8")
        req = urllib.request.Request(
            APPS_SCRIPT_URL,
            data=data,
        ...
        if "OK" in respuesta.upper():
            st.success("✅ ENTREVISTA GUARDADA EN GOOGLE SHEETS.")
            return True
        st.error(f"Google Sheets respondió: {respuesta}")
        return False
    except Exception as e:
...
def dato_limpio(datos, *claves):
    """
    Obtiene un valor desde datos usando varias claves posibles y limpia HTML.
    """
    for clave in claves:
        if clave in datos and str(datos.get(clave, "")).strip():
            try:
                return limpiar_html_editor(datos.get(clave, ""))
            except Exception:
                return str(datos.get(clave, "")).strip()
    return ""
...
def rellenar_docx_por_tablas(datos):
    ...
    if len(doc.tables) >= 6:
        try:
            ...
            # Tabla 5
            acuerdos_txt = dato_limpio(datos, "acuerdos", "Acuerdos")
            compromisos_txt = dato_limpio(datos, "compromisos", "Compromisos")
            texto_final = acuerdos_txt.strip()
            if compromisos_txt:
                texto_final += "\n\nCOMPROMISOS:\n" + compromisos_txt.strip()
...
def rellenar_docx_estudiante(datos):
    ...
    if len(doc.tables) >= 5:
        try:
            ...
            acuerdos_txt = dato_limpio(datos, "acuerdos", "Acuerdos")
            compromisos_txt = dato_limpio(datos, "compromisos", "Compromisos")
            acuerdos_finales = acuerdos_txt.strip()
...
def guardar_registro(datos):
    ...
    1. Google Sheets como almacenamiento principal permanente.
    2. Excel como espejo/descarga/respaldo de la sesión.
    """
    def _limpio(valor):
        try:
            return limpiar_html_editor(valor)
        except Exception:
            return str(valor or "").strip()

    def _desde_session(*keys):
        for k in keys:
            if k in st.session_state and str(st.session_state.get(k, "")).strip():
                return _limpio(st.session_state.get(k, ""))
        return ""
    ...
    registro = {
        ...
        "Antecedentes": _limpio(
            datos.get("antecedentes", "")
            or datos.get("Antecedentes", "")
            or datos.get("antecedentes_para_word", "")
            or _desde_session(
                "hp_antecedentes_ordenados_editor",
                "e_antecedentes_ordenados_editor",
                "antecedentes_hogar",
                "antecedentes_estudiante",
                "antecedentes_ordenados_hogar",
                "antecedentes_ordenados_estudiante"
            )
            or datos.get("motivo_word", "")
        ),
        "Acuerdos": _limpio(
            datos.get("acuerdos", "")
            or datos.get("Acuerdos", "")
            or datos.get("acuerdos_word", "")
            or _desde_session(
                "hp_acuerdos_editor",
                "e_acuerdos_editor",
                "acuerdos_hogar",
        ...
        "Compromisos": _limpio(
            datos.get("compromisos", "")
            or datos.get("Compromisos", "")
            or _desde_session(
                "hp_compromisos_editor",
                "e_compromisos_editor",
                "compromisos_hogar",
        ...
        "Resumen_libro": _limpio(
            datos.get("resumen_libro", "")
            or datos.get("Resumen_libro", "")
            or datos.get("resumen", "")
            or _desde_session("resumen_hogar", "resumen_estudiante")
        ),
        "Observaciones": _limpio(
            datos.get("observaciones", "")
            or datos.get("Observaciones", "")
            or _desde_session("observaciones_hogar", "observaciones_estudiante")
        ),
    ...
    # Validación visible de campos críticos antes de enviar a Google Sheets
    faltantes = []
    for campo in ["Antecedentes", "Acuerdos", "Compromisos"]:
        if not str(registro.get(campo, "")).strip():
            faltantes.append(campo)

    if faltantes:
        st.warning("Atención: se guardará la entrevista, pero estos campos están vacíos: " + ", ".join(faltantes))

    ok_google = enviar_registro_google_sheets(registro)

    try:
        if REGISTRO_EXCEL.exists():
            try:
                crear_respaldo_registro_entrevistas(motivo="antes_de_guardar")
            except Exception:
                pass
            df_registro = pd.read_excel(REGISTRO_EXCEL, sheet_name="Registro_Entrevistas").fillna("")
        else:
        ...
        if ok_google:
            st.success("Registro guardado en Google Sheets y espejo Excel actualizado.")
        else:
            st.warning("El registro quedó en el Excel temporal, pero NO fue confirmado en Google Sheets. Revise conexión o Apps Script.")

    except PermissionError:
        st.error("No se pudo guardar el espejo Excel porque registro_entrevistas.xlsx está abierto.")
    except Exception as e:
        st.error(f"No fue posible actualizar el espejo Excel: {e}")
...
def pagina_guardado_manual_google_sheets():
    encabezado("Guardado manual Google Sheets", "Plan de emergencia para registrar entrevistas sin perder información")
    st.warning("Use esta pantalla si el botón del formulario no aparece. Esto guarda directamente en Google Sheets.")

    with st.form("form_guardado_manual_google_sheets"):
        c1, c2, c3 = st.columns(3)
        with c1:
            fecha = st.date_input("Fecha entrevista", value=date.today())
            hora = st.text_input("Hora", value=datetime.now().strftime("%H:%M"))
            curso = st.text_input("Curso")
            estudiante = st.text_input("Estudiante")
        with c2:
            apoderado = st.text_input("Apoderado")
            motivo = st.text_input("Motivo")
            cc = st.text_input("CC")
            numero = st.text_input("Número entrevista")
        with c3:
            estado = st.text_input("Estado caso")
            protocolo = st.text_input("Protocolos aplicados")
            tipo = st.selectbox("Tipo registro", ["Entrevista Hogar / Apoderado", "Entrevista Estudiante", "Otro"])

        antecedentes = st.text_area("Antecedentes", height=180)
        acuerdos = st.text_area("Acuerdos", height=160)
        compromisos = st.text_area("Compromisos", height=140)
        resumen = st.text_area("Resumen libro", height=100)
        observaciones = st.text_area("Observaciones", height=100)

        enviar = st.form_submit_button("💾 GUARDAR AHORA EN GOOGLE SHEETS", type="primary", use_container_width=True)
    ...
    if enviar:
        datos_manual = {
            ...
            "motivo": motivo,
            "cc": cc,
            "numero_entrevista": numero,
            "estado_caso": estado,
            "protocolos_aplicados": protocolo,
            "tipo_registro": tipo,
            "antecedentes": antecedentes,
            "acuerdos": acuerdos,
            "compromisos": compromisos,
            "resumen_libro": resumen,
            "observaciones": observaciones,
        }
        guardar_en_google_sheets_directo(datos_manual)
...
if menu == "Inicio":
    ...
    acuerdos_final = limpiar_html_editor(acuerdos)
    compromisos_final = limpiar_html_editor(compromisos)
    antecedentes_final = limpiar_html_editor(antecedentes_para_word or antecedentes)
    resumen_libro_final = limpiar_html_editor(resumen_libro)
    observaciones_final = limpiar_html_editor(observaciones)

    datos = {
        **datos_previos,
        "antecedentes": antecedentes_final,
        "acuerdos": acuerdos_final,
        "compromisos": compromisos_final,
        "observaciones": observaciones_final,
        "resumen_libro": resumen_libro_final,
        "motivo_word": motivo_word,
    }

    st.divider()
    st.info("Para registrar oficialmente la entrevista, presione el botón verde de guardado del formulario.")

    col_guardar, col_word = st.columns(2)

    with col_guardar:
        if st.button("💾 GUARDAR ENTREVISTA", key="guardar_hogar", type="primary", use_container_width=True):
            if not estudiante:
    ...
    antecedentes_final = limpiar_html_editor(antecedentes_para_word or antecedentes)
    acuerdos_final = limpiar_html_editor(acuerdos_word_final)
    compromisos_final = limpiar_html_editor(compromisos)
    resumen_libro_final = limpiar_html_editor(resumen_libro)
    observaciones_final = limpiar_html_editor(observaciones)
    ...
    with col_guardar:
        if st.button("💾 GUARDAR ENTREVISTA", key="guardar_estudiante", type="primary", use_container_width=True):
            if not estudiante:
                st.error("Debes seleccionar estudiante.")
            elif datos.get("estado_caso") == "Cerrado" and datos.get("pendientes_checklist"):
                st.error("No se puede registrar como Cerrado porque existen pasos pendientes en el checklist.")
